...
class ClienteControllerTest {
    ...
    public void clienteCadastradoComSucesso() throws Exception {
        ...
        MockHttpServletResponse resposta =
                this.mvc.perform(MockMvcRequestBuilders.post("/clientes")
                        .contentType(MediaType.APPLICATION_JSON).content(this.dadosCadastroClienteJson.write(dadosCadastro).getJson()))
    ...
    public void listarClientesConformePaginacao() throws Exception {
        ...
        Mockito.when(this.dataManagerClient.listar(ArgumentMatchers.any(Pageable.class))).thenReturn(page);

        String respostaString =
                this.mvc.perform(MockMvcRequestBuilders.get("/clientes")
                        .contentType(MediaType.APPLICATION_JSON))
                        .andExpect(MockMvcResultMatchers.status().isOk())
                        .andReturn().getResponse().getContentAsString(StandardCharsets.UTF_8);

        JsonNode clientesNode = this.mapper.readTree(respostaString).get("content");
        List<DadosListagemCliente> clientesResposta =
                this.mapper.readValue(clientesNode.toString(), new TypeReference<>() {});
    ...
    public void atualizarCliente() throws Exception {
        ...
        String resposta = this.mvc.perform(MockMvcRequestBuilders.put("/clientes").contentType(MediaType.APPLICATION_JSON).content(this.dadosAtualizacaoClienteJson.write(dadosAtualizacao).getJson())).andExpect(MockMvcResultMatchers.status().isOk()).andReturn().getResponse().getContentAsString(StandardCharsets.UTF_8);
    ...
    public void excluirCliente() throws Exception {
        ...
        Mockito.doNothing().when(this.dataManagerClient).deletar(ArgumentMatchers.anyLong());
    ...
    public void retornarClientePorId() throws Exception {
        ...
        String resposta =
                this.mvc.perform(MockMvcRequestBuilders.get("/clientes/" + id))
    ...
    public void retornarScoreCredito() throws Exception {
        ...
        Mockito.when(this.dataManagerClient.getById(id)).thenReturn(cliente);
        Mockito.when(this.servicoCliente.calcularScoreCredito(cliente.saldoCc())).thenReturn(scoreEsperado);
        ...
        String respostaRequisicao =
                this.mvc.perform(MockMvcRequestBuilders.get("/clientes/" + id + "/score"))
                        .andReturn().getResponse().getContentAsString(StandardCharsets.UTF_8);
